...
function sesionChanged(sesion){
	...
	if(sesion == "selectOptionNew") {
		...
		sesionData.actualizarSesionPorDefecto(sesion)
            .catch(function() {
...
function getEstadisticasSesion(tiempos) {
    sesionData.getEstadisticas(tiempos)
        .then(estadisticas => pintarEstadisticas(estadisticas))
        .catch(() => console.error("No se han podido cargar las estadísticas"));
}

function getTiemposSesion(sesion) {
    originalSelectedOption = sesion;

    sesionData.getSolves(sesion)
        .then(json => {
            json = formatJsonTiempos(json, 0);
            getEstadisticasSesion(json);
            pintarTablaTiempos(json);
        });
}
...
function borrarUltimoTiempoMobile() {
    const select = document.getElementById("sesion_select");
    sesionData.borrarUltimoTiempoMobile(select.value)
        .then(data => {
            if(data.eliminado) {
                getSesiones();
...
function crearSesion(nombreSesion) {
	if(validarNombreSesion(nombreSesion)) {
	    sesionData.crearSesion(nombreSesion)
			.then(sesion => {
...
function borrarSesion(nombreSesion) {
    sesionData.borrarSesion(nombreSesion)
		.then(data => {
			if(data.eliminado) {
...
function actualizarSesion(newName) {
    ...
    sesionData.renombrarSesion(name, newName)
        .then(data => {
            if (data) {
...
function addMas2(id) {
	...
	sesionData.updateMas2(id, action)
        .then(data => {
            if (data.actualizado) {
                if (action == 0) {
                    document.getElementById("solveBtn-mas2").classList.remove("solveBtn-clicked");
...
function addDnf(id) {
	...
	sesionData.updateDnf(id, action)
        .then(data => {
            if (data.actualizado) {
                if (action == 0) {
                    document.getElementById("solveBtn-dnf").classList.remove("solveBtn-clicked");
...
function addMas2UltimoSolveMobile(action) {
	...
	sesionData.updateMas2Last(select.value, action)
        .then(data => {
            if (data.actualizado) {
                if (action == 0) {
                    document.getElementById("btn-mobile-delete").classList.remove("d-none");
...
function addDnfUltimoSolveMobile(action) {
	...
	sesionData.updateDnfLast(select.value, action)
        .then(data => {
            if (data.actualizado) {
                if (action == 0) {
                    document.getElementById("btn-mobile-delete").classList.remove("d-none");
...
function importSolves() {
    ...
    sesionData.importSolves(formData)
        .then(data => {
            if (data.importado) {
                getSesiones();
...
function guardarPreferencias(event) {
    ...
    sesionData.guardarPreferencias(event.target)
        .then(() => {
            window.location.reload();
        })
        .catch(error => {
            console.error(error);
            document.getElementById('config-modal-error').style.display = 'block';
        });
}
