    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (path) {
            case "/save":
                ...
                int segundosInspeccion = (request.getParameter("config-inspect-sec") != null && !request.getParameter("config-inspect-sec").isEmpty()) ? Integer.parseInt(request.getParameter("config-inspect-sec")) : 0;

                Conf conf = new Conf(tema, idioma, (ocultarElementos) ? 1 : 0, (ocultarPreview) ? 1 : 0, (pulsacionLarga) ? 1 : 0, (cronometroRaton) ? 1 : 0, (tiempoInspeccion) ? 1 : 0, segundosInspeccion);

                usuarioService.actualizarConfiguracionUsuario(usuario, conf);

                response.sendRedirect("/rubikTimerWeb");
                break;
            default:
                break;
        }
